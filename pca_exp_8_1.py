...
from fair_k_pca import get_fair_subspace, get_fair_subspace_MANUAL_2_GROUPS, get_fair_subspace_MANUAL, get_fair_subspace_MANUAL_2_GROUPS_COSINE
...
def intersect_subspaces(embeddings, subclasses, misclassified, k):
    u1, _, _ = get_fair_subspace(scale_G(embeddings), subclasses, k, [0, 1])
...
def out_of_dist_generalization():
    ...
    for k in [10, 20, 30, 40, 50, 60, 70]:
        # u_space, _, pca_fair = get_fair_subspace(scale_G(compute_pca(embeddings, 50, False)), labels, k, [0, 1])
        u_space = get_fair_subspace_MANUAL(0, 1, k, embeddings, subclasses, misclassified)
        # u_space, pca_fair = get_fair_subspace_MANUAL_2_GROUPS(1, 0, scale_G(compute_pca(embeddings, 50, False)), labels)
        # u_space = intersect_subspaces(embeddings, subclasses, misclassified, k)
        X_proj = X @ u_space
